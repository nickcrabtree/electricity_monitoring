...
- `sensor.electricity_power` in HA reads a constant 0 W. It is **not** a whole-house meter; there is no whole-house meter in HA or Graphite. `home.electricity.aggregate.power_watts` is the sum of monitored plugs only (~340 W with quartz idle, ~480 W with quartz training).
