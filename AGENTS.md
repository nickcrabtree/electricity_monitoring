...
- **Whole-house meter: an ESP32 in the garage reading the electricity meter's pulse LED**, firmware `~/code/home_assistant/imported_from_scripts/electricity_monitor/` (MicroPython; `monitor.py`, `config.py`, `LM393-migration-plan.md`). Design as of the 2025-11-09 migration plan: an LM393 light-comparator module on GPIO34 (originally a bare LDR on the ADC), falling-edge interrupt per flash, 4000 imp/kWh, Wi-Fi `12 Lloyd`. It publishes to **both** sinks: Graphite `home.electricity.meter.{power_watts,power_kw,total_kwh,pulse_count}` every 10 s (this is the `meter` series on the Grafana Usage and Whole House panels), and HA `sensor.electricity_consumption` / `sensor.electricity_power` every 5 min. **Status 2026-09-26: the board is alive but not counting.** It still reports every minute to Graphite and every 5 min to HA, so Wi-Fi and firmware are up; but `power_watts` has read 0 since 17:12 on 2026-09-24 and `pulse_count` last advanced at 13:46 on 2026-09-25 (one stray pulse), then 15,479,500 flat; `total_kwh` stuck at 3869.875. That pattern points at the sensor side (LM393 threshold pot / alignment with the meter's very faint LED, or the module itself), not at Wi-Fi or a crashed firmware. Nick's to-do: confirm which, re-aim/re-trim the sensor, or find a better source of whole-house data. Until then `sensor.electricity_power` (0 W) is **not** a usable house total, and `home.electricity.aggregate.power_watts` is the sum of monitored plugs only (~340 W with quartz idle, ~480 W with quartz training).
